...
4. Zvolte tlačítko **OK**. A new item card opens with some fields filled with information from the template.
...
>
> If you select **Average**, then the item's unit cost is calculated as the average unit cost at each point in time after a purchase. Zásoby se oceňují s předpokladem, že všechny zásoby se prodávají současně. With this setting, you can choose the **Unit Cost** field to view, on the **Average Cost Calc. Overview** page, the history of transactions that the average cost is calculated from.
